...
class Pemilih extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['pemilihbykelas'] = $this->pemilih_m->getByKelas($this->session->id_kelas);
    ...
    {
        $data['mPemilih'] = true;
        $data['pemilih'] = $this->pemilih_m->getByKelas($this->input->post('kelas',true));
        $data['kelas'] = $this->fakultas_m->getAll();
        $data['content'] = 'v_pemilih';
        $this->load->view('index',$data);
    }
    ...
    public function import(){
        ...
        if($this->upload->do_upload('import_file')){ 
            ...
            foreach($sheet as $row){
                $cek = $this->pemilih_m->cekNIS($row['A']);
                $nis = $this->pemilih_m->getNIS($id);
                $password = password_hash($this->input->post('password',true),PASSWORD_DEFAULT);
                ...
                if(!$cek) { 
                    if($numrow > 1){
                        $data = [
                            'id_kelas'=>$id_kelas,
                            'nis'=>$row['A'],//input data dari data excel kolom A
                            'nama'=>$row['B'],
                            'email'=>$row['C'],
            ...
            $this->session->set_flashdata('berhasil','Anda berhasil menambahkan <b>'.$success.'</b> data baru dan <b>'.$duplicate.'</b> memiliki NIS yang sama');  
        }else{  
    ...
    public function kirim_all($id_kelas){
        $email = $this->pemilih_m->getByKelas($id_kelas);

        foreach ($email as $row) {
            $this->_email($row['nis'],$row['nama'],$row['email']);
    ...
    public function kirim($id){
        ...
        $this->_email($email['nis'],$email['nama'],$email['email']);
    ...
    private function _email($nis,$nama,$email){
        $config = [
            'protocol'  =>'smtp',
        ...
        $this->email->message('
                    ...
                    Silahkan login pada sistem dengan menggunakan NIS dan Password yang anda daftarkan di link berikut : <br>
